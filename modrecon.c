...
void maximize() {
	...
	double pe0 ;
		...
		for (r = 0 ; r < num_rot ; ++r) {
			...
			for (t = 0 ; t < ones[d] ; ++t) 
				w_out_r[place_ones_d[t]] = p[r] * e[1] ;
			
			for (t = 0 ; t < multi[d] ; ++t)
				w_out_r[place_multi_d[t]] += p[r] * e[count_d[t]] ;
		}
	}
...
int setup() {
	...
	double z, expo ;
	e = malloc((1 + countmax) * sizeof(double)) ;
	expo = exp(mean_count / num_pix) ;
	
	if (frac == 0.)
		e[0] = mean_count * expo / (1 + expo) / num_pix ;
	else
		e[0] = mean_count * (frac + (1 + frac) * expo) / (1 + expo) / num_pix ;
	
	for (t = 1 ; t <= countmax ; ++t) {
		z = (1 + frac) / frac ;
		e[t] = mean_count * frac * (1 + expo * pow(z, (double) t + 1.)) / (1 + expo * pow(z, (double) t)) / num_pix ;
	}
